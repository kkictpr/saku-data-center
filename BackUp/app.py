...
if menu == "หน้าแรก (Portal Hub)":
    ...
    if st.button("⬅️ กลับหน้าแรก (Portal Hub)", key="back_home_verus"):
        ...
        st.rerun()
    st.markdown("---")
    st.title("⛏️ ระบบบริหารเหมืองขุด Verus (Mining Farm)")
    ...
    if sub_menu == "🛒 ขายสินค้า (POS Dashboard)":
    ...
    elif sub_menu == "📋 รายการสินค้า (Items Catalog)":
        st.title("📋 จัดการรายการสินค้า (Items Catalog)")
        
        with st.form("add_item_form"):
            col_i1, col_i2, col_i3 = st.columns(3)
            input_item_name = col_i1.text_input("ชื่อสินค้า", placeholder="พิมพ์ชื่อสินค้า...")
            selected_new_cat = col_i2.selectbox("หมวดหมู่สินค้า", options=["ไอศกรีม", "ท็อปปิ้ง", "เครื่องดื่ม", "อื่นๆ"])
            input_item_price = col_i3.text_input("ราคาขาย (บาท)", placeholder="0.0")
            
            if not os.path.exists("item_images"):
                os.makedirs("item_images")
            uploaded_file = st.file_uploader("🖼️ อัปโหลดรูปภาพสินค้า", type=["png", "jpg", "jpeg"])

            if st.form_submit_button("💾 บันทึกเพิ่มสินค้า"):
                parsed_price = safe_float(input_item_price)
                if not input_item_name or parsed_price is None or parsed_price < 0:
                    st.error("❌ กรุณากรอกชื่อและราคาให้ถูกต้อง!")
                else:
                    image_path_save = None
                    if uploaded_file is not None:
                        file_name = f"{int(time.time())}_{uploaded_file.name}"
                        image_path_save = os.path.join("item_images", file_name)
                        with open(image_path_save, "wb") as f:
                            f.write(uploaded_file.getbuffer())

                    conn = sqlite3.connect('datacenter.db', timeout=15)
                    c = conn.cursor()
                    c.execute("INSERT INTO ice_items (name, category, price, image_path) VALUES (?, ?, ?, ?)", 
                              (input_item_name.strip(), selected_new_cat, parsed_price, image_path_save))
                    conn.commit()
                    conn.close()
        ...
        st.markdown("---")
        st.subheader("📦 รายการสินค้าที่มีทั้งหมดในระบบ")
        try:
            conn = sqlite3.connect('datacenter.db', timeout=15)
